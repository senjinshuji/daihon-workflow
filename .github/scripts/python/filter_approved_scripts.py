...
def copy_script_file(script_file: str, product_name: str, output_dir: str, loop_number: int = 1) -> bool:
    """台本ファイルを承認ディレクトリにコピー"""
    print(f"  Attempting to copy: {script_file}")
    ...
    if len(parts) >= 2 and parts[0].startswith('writer'):
        ...
        # 優先順位順に可能性のあるパスを追加
        # 1. 新構造: loop_X/writerY/script_Z.md
        possible_paths.append(os.path.join(product_name, 'bulk_scripts', f'loop_{loop_number}', writer_name, script_name))
        # 2. 旧構造: loop_X/writer1_script_1.md
        possible_paths.append(os.path.join(product_name, 'bulk_scripts', f'loop_{loop_number}', script_file))

    # 3. bulk_scripts直下（最も一般的なケース）
    possible_paths.append(os.path.join(product_name, 'bulk_scripts', script_file))

    # 4. ループ番号なしのフォールバック
    if loop_number != 1:
        possible_paths.append(os.path.join(product_name, 'bulk_scripts', 'loop_1', script_file))
